...
class Op:
    ...
    def create_operators(self):
        ...
        # operator 1 +z
        self.M.append(np.matrix([[math.cos(da/2.0) - 1j*math.sin(da/2.0), 0.0], [0.0, math.cos(da/2.0) + 1j*math.sin(da/2.0)]]))
        self.name.append('+z');
        ...
        # operator 3 +w
        self.M.append(np.matrix([[math.cos(da/2.0) - 1j*math.cos(alpha)*math.sin(da/2.0), -math.sin(alpha)*math.sin(da/2.0)], [math.sin(alpha)*math.sin(da/2.0), math.cos(da/2.0) + 1j*math.cos(alpha)*math.sin(da/2.0)]]))
        self.name.append('+w');
    ...
    def check_unitarity(self):
        ...
        for k in range(len(self.M)):
            if (np.trace(self.M[k] * self.M[k].getH()) - 2 != 0):
                print "Operator " + self.name[k] + " (no. " + str(k) + ") isn't unitary!"
                exit()
    ...
    def determine_index_of_anti_operators(self):
        ...
        for k in range(len(self.M)):
           ...
           if found_operator == False:
               print "Couldn't find the anti-operator for operator " + self.name[k] + " (no " + str(k) + ")"

    def __str__(self):
        # just a test to play around
        hlpstr = ''
        for k in range(len(self.M)):
            hlpstr = hlpstr + self.name[k] + " " + str(self.anti_operator_no[k]) + "\n"
